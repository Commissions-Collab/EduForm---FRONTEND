...
const AttendanceMonthlySummary = () => {
  ...
  const calculateSummaryStats = () => {
    if (!monthlyAttendanceData?.students || !Array.isArray(monthlyAttendanceData.students)) {
      return {
        totalStudents: 0,
        totalDays: 0,
        presentCount: 0,
    ...
    students.forEach(student => {
      ...
      const studentSchoolDays = (summary.present_days || 0) + 
  ...
  const totalAttendanceRecords = summaryStats.presentCount + summaryStats.absentCount + summaryStats.halfDayCount;
  const attendanceRate = totalAttendanceRecords > 0
    ? (((summaryStats.presentCount + (summaryStats.halfDayCount * 0.5)) / totalAttendanceRecords) * 100).toFixed(1)
    : 0;
  ...
  const getDailyAttendanceData = () => {
    if (!monthlyAttendanceData?.students || !monthlyAttendanceData?.calendar_days) {
      return [];
    }
    ...
    return monthlyAttendanceData.calendar_days
      ...
      .map(day => {
        ...
        monthlyAttendanceData.students.forEach(student => {
          const dayAttendance = student.daily_attendance.find(att => att.date === day.date);
          if (dayAttendance) {
        ...
        const dayRate = totalStudents > 0 
          ? (((dayPresent + (dayHalfDay * 0.5)) / totalStudents) * 100).toFixed(1)
          : 0;
                ...
                style={{
                  width: `${
                    totalAttendanceRecords > 0
                      ? (summaryStats.presentCount / totalAttendanceRecords) * 100
                      : 0
                  }%`,
                }}
              ></div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
              <div
                ...
                style={{
                  width: `${
                    totalAttendanceRecords > 0
                      ? (summaryStats.absentCount / totalAttendanceRecords) * 100
                      : 0
                  }%`,
                }}
              ></div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
              <div
                ...
                style={{
                  width: `${
                    totalAttendanceRecords > 0
                      ? (summaryStats.halfDayCount / totalAttendanceRecords) * 100
                      : 0
                  }%`,
                }}
              ></div>
      ...
      {loading ? (
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Please ensure a section and academic year are selected and attendance data exists for this month.
          </p>
        </div>
      )}

      {/* Student List with Individual Stats */}
      {monthlyAttendanceData?.students && monthlyAttendanceData.students.length > 0 && (
                      ...
                      <div className="flex items-center">
                        <div className="ml-4">
                          ...
                          <div className="text-sm text-gray-500">
                            {studentData.student.student_id} • LRN: {studentData.student.lrn}
                          </div>
                        </div>
                      </div>
                    </td>
