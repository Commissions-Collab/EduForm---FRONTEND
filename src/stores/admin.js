...
export const useAdminStore = create((set, get) => ({
  ...
  updateGrade: async ({
    ...
    try {
      ...
      // Use PUT method to match your existing routes
      const { data } = await axiosInstance.put(
        "/teacher/academic-records/update-grade",
        payload
      );

      // Handle response format - extract grade data
      ...
      const updatedStudents = get().students.map((student) => {
        ...
        // Recalculate student status after grade update
        const filledGrades = updatedGrades.filter(
      ...
      if (err.response?.status === 403) {
        throw new Error("You don't have permission to edit this grade");
  ...
  updateMultipleGrades: async (gradeUpdates) => {
    try {
      const processedGrades = gradeUpdates.map(
        ({ student_id, subject_id, quarter_id, academic_year_id, grade }) => ({
          student_id: Number(student_id),
          subject_id: Number(subject_id),
          quarter_id: Number(quarter_id),
          academic_year_id: Number(academic_year_id),
          grade: grade === "" || grade === undefined ? null : Number(grade),
        })
      );
      ...
      // Update local state for all changed grades
      const updatedStudents = get().students.map((student) => {
        const studentGradeUpdates = gradeUpdates.filter(
          (update) => update.student_id === student.id
        );

        if (studentGradeUpdates.length === 0) return student;

        const updatedGrades = student.grades.map((g) => {
          const gradeUpdate = studentGradeUpdates.find(
            (update) => update.subject_id === g.subject_id
          );
        ...
        // Recalculate student status after grade updates
        const filledGrades = updatedGrades.filter(
          (grade) => grade.grade !== null && grade.grade !== undefined
        );
      ...
      if (err.response?.status === 403) {
        throw new Error("You don't have permission to edit one or more grades");
      } else if (err.response?.status === 422) {
