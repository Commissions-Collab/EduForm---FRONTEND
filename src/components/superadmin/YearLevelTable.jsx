...
const YearLevelTable = ({ searchTerm }) => {
  const { yearLevels, deleteYearLevel } = useAcademicManagementStore();
  ...
  const filteredRecords = useMemo(
    () =>
      yearLevels.data.filter((level) =>
        level.name.toLowerCase().includes(searchTerm.toLowerCase())
      ),
    [yearLevels.data, searchTerm]
  );
  ...
  const handleEdit = (level) => {
    updateFormData("yearLevel", level);
    setSelectedYearLevelId(level.id);
    setIsModalOpen(true);
  };
      ...
      <div className="border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
        <div className="p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            {!yearLevels.isLoading && (
              <div className="text-sm text-gray-500">
                {filteredRecords.length}{" "}
                {filteredRecords.length === 1 ? "year level" : "year levels"}{" "}
                found
                {searchTerm && (
                  <span className="ml-1">
                    for "
                ...
                <td colSpan={3} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center">
                      <LuBookOpen className="w-6 h-6 text-red-600" />
                    </div>
                    <div>
                      <p className="font-medium text-red-900">
                        Failed to load year levels
                      </p>
